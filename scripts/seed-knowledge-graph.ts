...
const ROUTES: RouteDef[] = [
  ...
  { method: 'GET', path: '/api/coins/bundles', prd: '§2.7', feature: 'Coin Economy', purpose: 'List coin packs and whether each was bought this Act' },
  { method: 'GET', path: '/api/orgs/:orgId/dashboard', prd: '§6.3', feature: 'Org Dashboard UI', purpose: 'Organization dashboard data' },
  { method: 'GET', path: '/api/orgs/:orgId/treasury', prd: '§6.3', feature: 'Org Treasury', purpose: 'Internal treasury health for the GitHub App installer or org admin' },
  ...
  { method: 'POST', path: '/api/acts/reset', prd: '§7.5', feature: 'Act Reset Job', purpose: 'Enqueue a forced Act reset (development or platform admin)' },
...
const REDIS_USE_CASES: RedisUseCase[] = [
  { name: 'Global Leaderboard', dataStructure: 'Sorted Set', keyPattern: 'leaderboard:global:{actId}', ttl: 'None (hydrate from DB if evicted)', feature: 'Tiers & Leaderboards' },
  { name: 'Org Leaderboard', dataStructure: 'Sorted Set', keyPattern: 'leaderboard:org:{orgId}:{actId}', ttl: 'None (hydrate from xp_logs if evicted)', feature: 'Tiers & Leaderboards' },
  { name: 'Leaderboard page cache', dataStructure: 'String JSON', keyPattern: 'cache:leaderboard:visible:{scope}:{actId}', ttl: '15s via LEADERBOARD_CACHE_TTL_SECONDS', feature: 'Tiers & Leaderboards' },
...
const FEATURES: Feature[] = [
  ...
  { name: 'XP Scoring Engine', prd: '§2.4', phase: 'Backend Core', status: 'done',
    ...
    roles: ['Contributor', 'Maintainer'],
    difficulties: ['Easy', 'Medium', 'Hard'],
    note: 'Live: PR #2 HARD 100-cap × (4.0/5) × 0.5 trust = 40 XP; Act 1 auto-created; Redis global+org scores; 0-star trust brackets no longer match every repo' },
  { name: 'Seasonal System (Acts)', prd: '§2.5', phase: 'Backend Core', status: 'done',
    ...
    note: 'Live: Act 1 45-day window; GET /api/acts/current returns days_remaining; dashboard Act card; daily reset check; Unranked+XP compression coded, not force-run on live Act 1' },
  { name: 'Maintainer Evaluation', prd: '§2.4', phase: 'Backend Core', status: 'done',
    ...
    roles: ['Maintainer'],
    note: 'Live: installer-only POST /api/prs/:id/evaluate; 5 sliders; dashboard pending card; merge stays AWAITING_EVALUATION until submit; XP awarded in-process' },
  { name: 'Tiers & Leaderboards', prd: '§2.6', phase: 'Backend Core', status: 'done',
    ...
    note: 'Live: Unranked hidden until first Act merge; Redis global+org + 15s page cache; hydrates after LRU eviction; ArunKushhhh Act 1 #1 40 XP INITIATOR' },
  { name: 'Coin Economy', prd: '§2.7', phase: 'Backend Core', status: 'done',
    components: ['api', 'worker'], dependsOn: ['Database Schema'],
    skills: ['engineering-skills:senior-backend', 'engineering-skills:tdd-guide'],
    services: ['CoinService'], metrics: ['pullquest_coins_minted_total'],
    roles: ['Contributor'],
    note: 'Live: 150 signup; GET /api/coins/bundles once-per-Act flags; checkout 409 if pack already bought; monthly mint skips duplicates this month; purchased never diluted' },
  { name: 'Org Treasury', prd: '§2.7', phase: 'Backend Core', status: 'done',
    components: ['api', 'worker'], dependsOn: ['Coin Economy'],
    skills: ['engineering-skills:senior-backend'],
    services: ['TreasuryService'], metrics: ['pullquest_treasury_balance'],
    roles: ['Organization'],
    note: 'Live: PullQuestTest 0 PC healthy; installer GET /api/orgs/:id/treasury; public dashboard has no raw balance; ceiling −2000; audit enqueued after credit/debit' },
  { name: 'Credibility Score', prd: '§2.9', phase: 'Backend Core', status: 'partial',
    ...
    roles: ['Contributor', 'Organization'],
    note: 'Public 0–100 score; raw treasury balance not public; treasury health input is live (PullQuestTest 0 PC)' },
  ...
  { name: 'Webhook Processing Queue', prd: '§6.6', phase: 'Worker & Jobs', status: 'partial',
    ...
    note: 'HMAC + BullMQ live; installation.created persists repos; issues + pull_request handlers verified on PullQuestTest (stake + merge + evaluation)' },
  { name: 'Act Reset Job', prd: '§7.5', phase: 'Worker & Jobs', status: 'partial',
    ...
    metrics: ['act_reset_duration_seconds', 'users_processed_total'],
    note: 'Daily 00:00 check; force via POST /api/acts/reset in development; refuses LOCKED stakes; archives Redis then UNRANKED+XP compress; not yet run on an expired Act' },
  { name: 'Monthly Coin Minting', prd: '§6.6', phase: 'Worker & Jobs', status: 'partial',
    components: ['worker'], dependsOn: ['Coin Economy'],
    skills: ['engineering-skills:senior-backend'],
    services: ['CoinService'], metrics: ['pullquest_coins_minted_total'],
    note: 'Cron 0 0 1 * *; skips users who already have MONTHLY_MINT this UTC month' },
  { name: 'Treasury Audit Job', prd: '§6.6', phase: 'Worker & Jobs', status: 'partial',
    components: ['worker'], dependsOn: ['Org Treasury'],
    skills: ['engineering-skills:senior-backend'],
    services: ['TreasuryService'], metrics: ['pullquest_treasury_balance'],
    note: 'Worker listens on treasury-audit; API enqueues check-debt-ceiling after credit/debit; no live breach yet (PullQuestTest at 0)' },
  ...
  { name: 'Dashboard Page', prd: '§4.1', phase: 'Frontend', status: 'partial',
    ...
    roles: ['Contributor'],
    note: 'Connect Repositories + pending eval + Act clock + global rank + internal org treasury card; realtime still pending' },
  ...
  { name: 'Leaderboard Pages', prd: '§2.6', phase: 'Frontend', status: 'done',
    components: ['web'], dependsOn: ['Tiers & Leaderboards'],
    skills: ['vercel:nextjs', 'dataviz'],
    roles: ['Contributor', 'Organization'],
    note: 'Live: Global + org tabs; your rank banner; Act 1 #1 INITIATOR 40 XP; Unranked copy until first merge' },
  ...
  { name: 'Coin Purchase UI', prd: '§2.7', phase: 'Frontend', status: 'done',
    components: ['web'], dependsOn: ['Stripe Payments'],
    skills: ['vercel:nextjs', 'engineering-skills:stripe-integration-expert'],
    roles: ['Contributor'],
    note: 'Dashboard Coin Store loads catalog from API; purchased-this-Act packs disabled; 409 copy if checkout repeats' },
  ...
  { name: 'Stripe Payments', prd: '§5', phase: 'Integrations', status: 'partial',
    ...
    roles: ['Contributor', 'Organization'],
    note: 'Coin checkout + once-per-Act 409; org subscription still incomplete' },
